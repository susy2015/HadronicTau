...
// Add cleanJets function
  ...
  while(tr->getNextEvent()){
    ...
    if(W_tau_prongsVec.size()!=0){
      ...
      myBaseHistgram.hTruecutFlow->Fill("muonVeto", passMuonVeto_tru * Lumiscale);
      myBaseHistgram.hTruecutFlow->Fill("eleVeto", passMuonVeto_tru * passEleVeto_tru * Lumiscale);
      myBaseHistgram.hTruecutFlow->Fill("isotrackVeto", passMuonVeto_tru * passEleVeto_tru * passIsoTrkVeto_tru * Lumiscale);
      myBaseHistgram.hTruecutFlow->Fill("nJets", passMuonVeto_tru * passEleVeto_tru * passnJets_tru * Lumiscale);
      myBaseHistgram.hTruecutFlow->Fill("dPhis", passMuonVeto_tru * passEleVeto_tru * passnJets_tru * passdPhis_tru * Lumiscale);
      myBaseHistgram.hTruecutFlow->Fill("met", passMuonVeto_tru * passEleVeto_tru * passnJets_tru * passdPhis_tru * passMET_tru * Lumiscale);
      myBaseHistgram.hTruecutFlow->Fill("bJets", passMuonVeto_tru * passEleVeto_tru * passnJets_tru * passdPhis_tru * passMET_tru * passBJets_tru * Lumiscale);
      myBaseHistgram.hTruecutFlow->Fill("tagger", passMuonVeto_tru * passEleVeto_tru * passnJets_tru * passdPhis_tru * passMET_tru * passBJets_tru * passTagger_tru * Lumiscale);
      myBaseHistgram.hTruecutFlow->Fill("ht", passMuonVeto_tru * passEleVeto_tru * passnJets_tru * passdPhis_tru * passMET_tru * passBJets_tru * passTagger_tru * passHT_tru * Lumiscale);
    }
    ...
    */
    ...
    if(W_tau_prongsVec.size() !=0 && passBaselineClosure){
	...
	const double corrBRTauToMu = kSR==-1 ? (1-Efficiency::MT2Bin_taumucorr()) :(1- Efficiency::taumucorTT(kSR));//correction from tauonic mu contamination   
	...
	const double corrMuAcc = 1./Efficiency::accTT(Efficiency::Njetbin(combNJetPt30Eta24), Efficiency::MT2bin(MT2_pre)); // Correction for muon acceptance
	...
	if( pass_mtw && passhtpred){
	  ...
	  if( kSR!=-1) {
	    ...
	    //	  const double SBcorr = corrBRWToTauHad * corrMuAcc * corrMuRecoEff * corrMuIsoEff * corrmtWEff * weight;
	...
	if( !istaumu_genRecoMatch && passhtpred){
	  myBaseHistgram.hnomtW_wt->Fill(nSB+1, Evt_weight);
	  if(pass_mtw) myBaseHistgram.hmtW_wt->Fill(nSB+1, Evt_weight);
	    
	  if(cnt1CSVS>=1 && nTopCandSortedCnt_pre>=1 && simmet>=200 &&  MT2_pre<200){
	    myBaseHistgram.hnomtW_wt->Fill(nSB, Evt_weight);
	    if(pass_mtw) myBaseHistgram.hmtW_wt->Fill(nSB, Evt_weight);   
	  }
	...
	if( passhtpred){
	  myBaseHistgram.hnotaumu_wt->Fill(nSB+1, Evt_weight);
	  if(istaumu_genRecoMatch) myBaseHistgram.htaumu_wt->Fill(nSB+1, Evt_weight);
	  
	  if(cnt1CSVS>=1 && nTopCandSortedCnt_pre>=1 && simmet>=200 &&  MT2_pre<200){
	    myBaseHistgram.hnotaumu_wt->Fill(nSB, Evt_weight);
	    if(istaumu_genRecoMatch) myBaseHistgram.htaumu_wt->Fill(nSB, Evt_weight);
	  }
