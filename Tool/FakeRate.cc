...
int main(int argc, char* argv[]) {
  ...
  TChain *fChain = 0;
  cout<<"file number: "<<allSamples[subsamplename].filelist_.size()<<endl;
  if(argc==6){
  ...
  for(int ib =1; ib <myBaseHistgram.hEff_Pt->GetNbinsX(); ib++){
    cout<<myBaseHistgram.hEff_Pt->GetBinContent(ib)<<endl;
  }
