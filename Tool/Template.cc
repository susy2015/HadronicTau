...
int main(int argc, char* argv[]) {
  ...
  while(tr.getNextEvent()){
    ...
    std::vector<double>genhadtaupt = tr.getVec<double>("genhadtaupt");
    std::vector<double>genhadtaueta = tr.getVec<double>("genhadtaueta");
    std::vector<double>genhadtauphi = tr.getVec<double>("genhadtauphi");
    std::vector<double>genvisiblehadtaueta = tr.getVec<double>("genvisiblehadtaueta");
    std::vector<double>genvisiblehadtauphi = tr.getVec<double>("genvisiblehadtauphi");
    ...
    // Select only events where the W decayed into a hadronically decaying tau
        ...
        if(genhadtaupt.size()==0)continue;//To skip rare events where same tau decays both into hadron and lepton!

    // Kinematic variables of generator-level tau
    const double genTauPt = genhadtaupt.at(0);
    const double genTauEta = genhadtaueta.at(0);
    const double genTauPhi = genhadtauphi.at(0);
    const double genvisibleTauEta = genvisiblehadtaueta.at(0);
    const double genvisibleTauPhi = genvisiblehadtauphi.at(0);
