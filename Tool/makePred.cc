...
// if pred_local_avg = 0 then force data_mu =1 and data_ele =1 to calculate the relative syst unc.
    if( relSys_for_ZERO )
    {
      ...
      const double pred_local_avg_rel_syst_SF_dn_err = pred_local_avg == 0 ? sqrt( std::abs(fin_TF_local_to_mu_SFdn - fin_TF_local_to_mu)*std::abs(fin_TF_local_to_mu_SFdn - fin_TF_local_to_mu) + std::abs(fin_TF_local_to_ele_SFdn - fin_TF_local_to_ele) * std::abs(fin_TF_local_to_ele_SFdn - fin_TF_local_to_ele) ) : pred_local_avg_syst_SF_dn_err/pred_local_avg;
